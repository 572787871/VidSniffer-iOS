...
class VideoSniffer {
  ...
  Future<List<VideoResource>> parsePage(String input) async {
    final pageUri = normalizeUrl(input);
    if (pageUri == null) {
      throw FormatException('URL 格式不正确：$input');
    }

    final direct = resourceFromUrl(pageUri.toString(), pageTitle: pageUri.host, source: '直接链接');
    if (direct != null) {
      return [direct];
    }
    ...
    return scanHtml(html, pageUri);
  }

  List<VideoResource> scanHtml(String html, Uri pageUri) {
    final title = _pageTitle(html) ?? pageUri.host;
    final candidates = <String>[];
    final patterns = <RegExp>[
      RegExp(r'''https?:[^"'\\\s<>]+?\.(?:m3u8|mp4|m4v|mov|ts)(?:\?[^"'\\\s<>]*)?''', caseSensitive: false),
      RegExp(r'''(?:src|href|url|file|video|source|content)["'\s:=]+([^"'\s<>]+?\.(?:m3u8|mp4|m4v|mov|ts)(?:\?[^"'\s<>]*)?)''', caseSensitive: false),
      RegExp(r'''"([^"]+?\.(?:m3u8|mp4|m4v|mov|ts)(?:\?[^"]*)?)"''', caseSensitive: false),
    ...
    for (final raw in candidates) {
      ...
      final resource = resourceFromUrl(resolved.toString(), pageTitle: title, pageUrl: pageUri.toString(), source: '网页源码');
  ...
  VideoResource? resourceFromUrl(String value, {String pageTitle = '网页视频', String pageUrl = '', String source = '自动嗅探'}) {
    final uri = normalizeUrl(value);
  ...
  bool _isAllowedMedia(Uri uri) {
    ...
    return ['.mp4', '.m4v', '.mov', '.m3u8', '.ts'].any(value.contains);
  }
